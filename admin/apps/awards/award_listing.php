...
$url="index.php?page=award_listing";
	if(isset($_GET['pgid']))
	{
	$currpg = $_GET['pgid'];
	}
...
<?php	
	...
	{ ?>
			<tr><td><a href="index.php?page=award&id=<?php echo $row['pkaward_id']; ?>"><?php echo $row['awd_recipient']; ?> </a></td>
				...
				<td><form name="edit_app" method="post" action="index.php?page=award&id=<?php echo $row['pkaward_id']; ?>"> 
					<input class="btton" type="submit" value="Edit Award" />
				</form>
				</td>
				<td><form name="new_inscrption" method="post" action="index.php?page=award_inscription&id=<?php echo $row['pkaward_id']; ?>"> 
					<input class="btton" type="submit" value="New Inscription" />
				</form>
				</td>
			</tr>
<?php	
